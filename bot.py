...
BASE_DIR = Path(file).parent.resolve()
...
ᴄʜᴏᴏsᴇ ᴀɴ ᴀᴄᴛɪᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ:
""")
keyboard = InlineKeyboardMarkup([
...
dur = f" ⏱{r['duration']//60}:{r['duration']%60:02d}" if r.get('duration') else ""
title = r['title'][:45] + "..." if len(r['title']) > 45 else r['title']
keyboard.append([InlineKeyboardButton(f"🎵 {title}{dur}", callback_data=f"song{i}")])
